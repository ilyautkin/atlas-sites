...
`write_file` и `deploy_folder` создают бэкап перед изменением. Логика единая для файлов (`.backup`) и папок (`.backup.tar.gz`):
...
`deploy_folder` создаёт `папка.backup.tar.gz` рядом с папкой на сервере перед заливкой (если бэкапа нет или он устарел).
