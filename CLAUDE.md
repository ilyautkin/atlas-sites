...
`write_file` и `deploy_folder` создают бэкап перед изменением. Имя — с префиксом `atlbkp-`: файлы `atlbkp-<имя>.backup`, папки `atlbkp-<папка>.backup.tar.gz`. Логика единая:

- Бэкап сегодняшний → оставить как есть
- Бэкап вчерашний или старше → удалить и создать свежий
- Бэкапа нет → создать (если файл/папка существуют)
...
| `create_directory(domain, path)` | домен, путь | Создать директорию (mkdir -p, путь от httpdocs/) |
...
| Команда | Параметры | Описание |
|---------|-----------|----------|
| `sync_folder(domain, remote_path, local_path)` | домен, путь на сервере, локальный путь | Скачать папку с сервера локально (pull) |
| `deploy_folder(domain, local_path, remote_path)` | домен, локальный путь, путь на сервере | Загрузить папку на сервер (push, автобэкап) |

Пути `remote_path` / `local_path` — **от домашней директории пользователя** (не от httpdocs/), например `httpdocs/core/components/commerce`.

Папка помещается **внутрь** целевого пути:
- `sync_folder(..., "httpdocs/core/components/commerce", "/tmp")` → `/tmp/commerce/`
- `deploy_folder(..., "/tmp/commerce", "httpdocs/core/components")` → `httpdocs/core/components/commerce/`

`deploy_folder` создаёт `atlbkp-папка.backup.tar.gz` рядом с папкой на сервере перед заливкой (если бэкапа нет или он устарел).

Все бэкапы (файлы и папки) именуются с префиксом **`atlbkp-`** → легко найти/удалить: `find / -name 'atlbkp-*'`.
...
### Чанки и категории (БД)

| Команда | Параметры | Описание |
|---------|-----------|----------|
| `create_chunk(domain, name, description, static_file, category)` | домен, имя, описание, путь к файлу, ID категории | Создать **файловый** (static) чанк. Ошибка, если чанк с таким именем уже есть |
| `update_chunk(domain, name, content)` | домен, имя, новый контент | Обновить `snippet` существующего чанка по имени. Для **не-статичных** чанков в БД (напр. theme `header`/`footer`). Вернёт warning, если чанк static |
| `find_category(domain, name)` | домен, имя | Найти ID категорий (`modx_categories`) по имени. Точные совпадения — первыми. Нужен для `create_chunk` (напр. `Repeater`, `Overview`) |

`create_chunk` — для кастомных item-tpl, регистрируемых как файловые чанки. `update_chunk` —
для правки контента чанков, которые тема держит в БД (static=0). `find_category` — чтобы
получить числовой `category` ID перед `create_chunk`.
...
### Работа с компонентом (папкой)
1. **Pull:** `sync_folder(domain, remote_path, local_path)` — скачать текущую версию с сервера
2. Редактировать файлы локально
3. **Push:** `deploy_folder(domain, local_path, remote_path)` — загрузить на сервер (бэкап создастся автоматически)
